...
sed -i -r 's/(#|^)ADDRESS .*/ADDRESS '"$DB_HOST"'/' /roachstation/config/dbconfig.txt
sed -i -r 's/(#|^)FEEDBACK_LOGIN .*/FEEDBACK_LOGIN '"$DB_USER"'/' /roachstation/config/dbconfig.txt
sed -i -r 's/(#|^)FEEDBACK_PASSWORD .*/FEEDBACK_PASSWORD '"$DB_PASS"'/' /roachstation/config/dbconfig.txt
...
envvar_override "TG_GAME_" "/roachstation/config/game_options.txt"
...
cd /roachstation
DreamDaemon roachstation.dmb -port 1337 -trusted -close -verbose
