...
// 	vardatum/action/cooldown/spell
...
// */
// datum/action/cooldown/spell/proc/cast_check(charge_check = TRUE, start_recharge = TRUE, mob/user = usr) //checks if the spell can be cast based on its settings; skipcharge is used when an additional cast_check is called inside the spell
...
//  */
// datum/action/cooldown/spell/proc/valid_target(target, user)
// 	return TRUE
...
//  */
// datum/action/cooldown/spell/proc/spend_spell_cost(mob/user)
...
// datum/action/cooldown/spell/proc/invocation(mob/user) //spelling the spell out and setting it on recharge/reducing charges amount
...
// datum/action/cooldown/spell/proc/playMagSound()
...
// datum/action/cooldown/spell/Click()
// 	if(cast_check(TRUE, FALSE, usr))
// 		choose_targets(usr)
// 	return 1

// datum/action/cooldown/spell/InterceptClickOn(mob/user, params, atom/A)
// 	. = ..()
// 	if(.)
// 		return
...
//  */
// datum/action/cooldown/spell/proc/choose_targets(mob/user)
...
//  * * user - The caster of the spell
//  */
// datum/action/cooldown/spell/proc/try_perform(list/targets, mob/user)
// 	SHOULD_NOT_OVERRIDE(TRUE)
// 	if(!length(targets))
...
//  * * user - The caster of the spell
//  */
// datum/action/cooldown/spell/proc/perform(list/targets, recharge = TRUE, mob/user = usr) //if recharge is started is important for the trigger spells
// 	SHOULD_NOT_OVERRIDE(TRUE)
...
//  */
// datum/action/cooldown/spell/proc/write_custom_logs(list/targets, mob/user)
// 	return


// datum/action/cooldown/spell/proc/before_cast(list/targets, mob/user)
// 	SHOULD_CALL_PARENT(TRUE)
// 	if(overlay)
...
// datum/action/cooldown/spell/proc/after_cast(list/targets, mob/user)
// 	SHOULD_CALL_PARENT(TRUE)
// 	for(var/atom/target in targets)
// 		var/location
// 		if(istype(target,/mob/living))
...
//  * * user - The caster of the spell
//  */
// datum/action/cooldown/spell/proc/cast(list/targets, mob/user = usr)
// 	return

// datum/action/cooldown/spell/proc/critfail(list/targets)
// 	return
...
// datum/action/cooldown/spell/proc/updateButtonIcon()
// 	if(action)
// 		action.UpdateButtonIcon()

// datum/action/cooldown/spell/proc/adjust_var(mob/living/target = usr, type, amount) //handles the adjustment of the var when the spell is used. has some hardcoded types
// 	switch(type)
// 		if("bruteloss")
...
// datum/action/cooldown/spell/aoe_turf/write_custom_logs(list/targets, mob/user)
// 	add_attack_logs(user, null, "Cast the AoE spell [name]", ATKLOG_ALL)

// datum/action/cooldown/spell/proc/can_cast(mob/user = usr, charge_check = TRUE, show_message = FALSE)
...
// 		var/clothcheck = locate(datum/action/cooldown/spell/noclothes) in user.mob_spell_list
// 		var/clothcheck2 = user.mind && (locate(datum/action/cooldown/spell/noclothes) in user.mind.spell_list)
...
// datum/action/cooldown/spell/summonmob/cast(list/targets, mob/user = usr)
// 	if(!target_mob)
// 		return
